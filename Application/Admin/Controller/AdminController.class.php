...
;    }
    ...
    */
    ...
    public function wxupload() {
        ...
        if(!empty(I('post.gift_id', ''))){
            /*M('carte_info')->where('menu_id="'.$_POST['menu_id'].'"')->field('menu_pag_url')->delete();*/
            M('gift_info')->where('gift_id="'.$_POST['gift_id'].'"')->setField('gift_url','');
            M('gift_info')->where('gift_id="'.$_POST['gift_id'].'"')->setField('gift_url',$save_path);
            $id=I('post.menu_id', '');
        }else{
    ...
    public function editGift(){
        if(IS_POST) {
            ...
            //var_dump($data);exit();
            if(M('gift_info')->where('gift_id = "'.$gift_id.'"')->save($data)){
                $this->success('修改成功', U('gift',array('sid'=>$sid)));
                exit;
            }else{
                $this->error('修改失败');
            }
        ...
        $where['sid']=$_GET['sid'];
        $data = M('gift_info')->fetchSql(false)->where($where)->find();
        //echo M('meal_info')->_sql();
        $wechat = D('Wechat');
        $wx_config = $wechat -> wx_get_sign_config();
        $this -> assign('wx_config', $wx_config);
        ...
        $this->assign('openid',$_GET['openid']);
        $this->assign('sid',$_GET['sid']);
        $this->assign('gift_id',$_GET['gift_id']);
        $this->assign('data',$data);
        $this->display();
    }
    ...
    */
    ...
    public function editFood(){
        if(IS_POST) {
            ...
            //var_dump($data);exit();
            if(M('carte_info')->where('menu_id = "'.$menuid.'"')->save($data)){
