function Ship(ctx){
	gameMonitor.im.loadImage(['/Public/quanxian/img/game/player.png']);
	this.width = 80;
	this.height = 80;
	this.left = gameMonitor.w/2 - this.width/2;
	this.top = gameMonitor.h - 2*this.height;
	this.player = gameMonitor.im.createImage('/Public/quanxian/img/game/player.png');
			...
			if(f){
				...
				if(l3<=this.height/2 + f.height/2){
					...
					if(f.type==0){
						...
						setTimeout(function(){
							// 得到分数赋值							
							var sscore = $('#score').text();
							$('#gameCard').attr('href','/admin/waiter/gameCard?score='+sscore);
							$('#gameoverPanel').hide();
							$('#resultPanel').show();
...
function Food(type, left, id){
	...
	var p = this.type == 0 ? '/Public/quanxian/img/game/food1.png' : '/Public/quanxian/img/game/food2.png';
...
var gameMonitor = {
	...
	init : function(){
		...
		bg.src = '/Public/quanxian/img/game/bg.jpg';
