function Ship(ctx){
	gameMonitor.im.loadImage(['/xhy/public/images/game/player.png']);
	this.width = 80;
	this.height = 80;
	this.left = gameMonitor.w/2 - this.width/2;
	this.top = gameMonitor.h - 2*this.height;
	this.player = gameMonitor.im.createImage('/xhy/public/images/game/player.png');
				...
				if(l3<=this.height/2 + f.height/2){
					...
					if(f.type==0){
						...
						setTimeout(function(){
							...
							$('#gameCard').attr('href','/admin/index/gameCard?score='+sscore);
							$('#gameoverPanel').hide();
							$('#resultPanel').show();
...
function Food(type, left, id){
	...
	var p = this.type == 0 ? '/xhy/public/images/game/food1.png' : '/xhy/public/images/game/food2.png';
...
var gameMonitor = {
	...
	init : function(){
		...
		bg.src = '/xhy/public/images/game/bg.jpg';
