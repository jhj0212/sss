$.fn.bMap = function(h) { h = $.extend({ name: "map" }, h);
    var j = $(this),
        b = h.name,
        p, m = 116.331398,
        l = 39.897445,
        q, a, n = $("<div class='map-warp' id='Map_" + b + "'></div"),
        o = $("<input type='text' name='" + b + "' placeholder='请输入门店具体位置' id='Map_input_" + b + "'/>"),
    ...
    if (h.location || h.address) {
        if (h.location) { p = new BMap.Point(h.location[0], h.location[1]);
            m = h.location[0];
            l = h.location[1];
            g.val(m + "," + l);
            if (!h.address) { c(p, null, false) } else { o.val(h.address) } } else { o.val(h.address);
        ...
        k.getCurrentPosition(function(t) {
            if (this.getStatus() == BMAP_STATUS_SUCCESS) {
                ...
                p = t.point;
                m = t.point.lng;
                l = t.point.lat } }, { enableHighAccuracy: true }) }
    o.focus(function(r) { n.show().css("zIndex", 999);
        d() });
    o.blur(function(r) { n.hide().css("zIndex", 0) });

    function d() {
        if (a) {
            return false }
        a = new BMap.Map("Map_" + b, { enableMapClick: false });
        ...
        q.addEventListener("onconfirm", function(v) {
            ...
            function s() {
                ...
                a.centerAndZoom(w, 17);
                c(w, t);
                e(w) }
            var u = new BMap.LocalSearch(a, { onSearchComplete: s });
            u.search(myValue) }) }
    ...
    function c(r, s, u) {
        ...
        t.getLocation(r, function(v) {
            ...
            if (q) { q.setInputValue(w.province + w.city + w.district + w.street + w.streetNumber + w.business) } else { t.getLocation(r, function(x) { o.val(x.address) }) }
            if (h.callback && !u) { h.callback(w, r) } }) }

    function e(r) { a.clearOverlays();
